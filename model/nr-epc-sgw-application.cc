...
void
NrEpcSgwApplication::AddEnb(uint16_t cellId, Ipv4Address enbAddr, Ipv4Address sgwAddr)
{
...
void
NrEpcSgwApplication::SendToS1uSocket(Ptr<Packet> packet, Ipv4Address enbAddr, uint32_t teid)
{
...
NrEpcSgwApplication::DoRecvModifyBearerRequest(Ptr<Packet> packet)
{
    ...
    for (auto& bearerContext : bearerContexts)
    {
        NS_ASSERT_MSG(bearerContext.fteid.interfaceType == NrGtpcHeader::S1U_ENB_GTPU,
