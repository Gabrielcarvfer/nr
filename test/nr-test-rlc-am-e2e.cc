 ...
 */
static NrRlcAmE2eTestSuite lteRlcAmE2eTestSuite;
...
NrRlcAmE2eTestCase::DoRun()
{
    ...
    // Note: Just one eNB and UE is supported. Everything is done in InstallGnbDevice and
    // InstallUeDevice
    ...
    //   enbNrDevs.Get (0)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback
    //   (&NrRlcAmE2eTestCase::EnbDropEvent, this));
    ...
    lteSimpleHelper->m_enbMac->SetTxOpportunityMode(NrTestMac::AUTOMATIC_MODE);

    // MAC sends transmission opportunities (TxOpp)
    lteSimpleHelper->m_ueMac->SetTxOppSize(ulTxOppSizeBytes);
    lteSimpleHelper->m_ueMac->SetTxOppTime(Seconds(ulTxOpprTimeSeconds));
    lteSimpleHelper->m_ueMac->SetTxOpportunityMode(NrTestMac::AUTOMATIC_MODE);

    // Start/Stop pseudo-application at RRC layer
    Simulator::Schedule(Seconds(sduStartTimeSeconds), &NrTestRrc::Start, lteSimpleHelper->m_enbRrc);
    Simulator::Schedule(Seconds(sduStopTimeSeconds), &NrTestRrc::Stop, lteSimpleHelper->m_enbRrc);
