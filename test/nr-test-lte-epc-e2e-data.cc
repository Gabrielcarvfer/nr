...
class NrEpcE2eDataTestCase : public TestCase
{
  public:
     ...
     */
    NrEpcE2eDataTestCase(std::string name, std::vector<EnbTestData> v);
...
NrEpcE2eDataTestCase::NrEpcE2eDataTestCase(std::string name, std::vector<EnbTestData> v)
    : TestCase(name),
      m_enbTestData(v)
{
...
NrEpcE2eDataTestCase::DoRun()
{
    ...
    NetDeviceContainer enbNrDevs = nrHelper->InstallGnbDevice(enbs, bandwidthAndBWPPair.second);
    nrHelper->UpdateDeviceConfigs(enbNrDevs);
    auto enbNrDevIt = enbNrDevs.Begin();

    uint16_t ulPort = 1000;

    for (auto enbit = m_enbTestData.begin(); enbit < m_enbTestData.end(); ++enbit, ++enbNrDevIt)
    {
        ...
        for (uint32_t u = 0; u < ues.GetN(); ++u)
        {
            ...
            Ipv4InterfaceContainer ueIpIface =
                epcHelper->AssignUeIpv4Address(NetDeviceContainer(ueNrDevice));
            ...
            nrHelper->AttachToEnb(ueNrDevice, *enbNrDevIt);
