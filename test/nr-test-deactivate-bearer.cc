    ...
    : TestSuite("nr-test-deactivate-bearer", Type::SYSTEM)
{
    ...
    std::vector<uint16_t> dist_1{0, 0, 0}; // 3 co-located UEs
...
NrDeactivateBearerTestCase::DoRun()
{
    ...
    enbDevs = nrHelper->InstallGnbDevice(enbNodes, bandwidthAndBWPPair.second);
    ...
    nrHelper->UpdateDeviceConfigs(enbDevs);
    nrHelper->UpdateDeviceConfigs(ueDevs);

    Ptr<NrGnbNetDevice> lteEnbDev = enbDevs.Get(0)->GetObject<NrGnbNetDevice>();
    Ptr<NrGnbPhy> enbPhy = lteEnbDev->GetPhy(0);
    ...
    for (int i = 0; i < m_nUser; i++)
    {
        ...
        Ptr<NrUeNetDevice> lteUeDev = ueDevs.Get(i)->GetObject<NrUeNetDevice>();
        Ptr<NrUePhy> uePhy = lteUeDev->GetPhy(0);
    ...
    {
        nrHelper->AttachToEnb(ueDevs.Get(i), enbDevs.Get(0));
    }
