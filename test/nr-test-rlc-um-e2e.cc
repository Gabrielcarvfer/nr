 ...
 */
static NrRlcUmE2eTestSuite lteRlcUmE2eTestSuite;
...
NrRlcUmE2eTestCase::DoRun()
{
    ...
    Ptr<NrSimpleHelper> lteSimpleHelper = CreateObject<NrSimpleHelper>();
    ...
    NetDeviceContainer enbNrDevs = lteSimpleHelper->InstallGnbDevice(enbNodes);
    NetDeviceContainer ueNrDevs = lteSimpleHelper->InstallUeDevice(ueNodes);

    // Note: Just one eNB and UE are supported. Everything is done in InstallGnbDevice and
    // InstallUeDevice

    // Attach one UE per eNodeB
    // for (uint16_t i = 0; i < numberOfNodes; i++)
    //   {
    //     lteSimpleHelper->Attach (ueNrDevs.Get(i), enbNrDevs.Get(i));
    //   }

    // lteSimpleHelper->ActivateEpsBearer (ueNrDevs, NrEpsBearer
    ...
    enbNrDevs.Get(0)->TraceConnectWithoutContext(
        "PhyRxDrop",
    ...
    lteSimpleHelper->m_enbMac->SetTxOpportunityMode(NrTestMac::RANDOM_MODE);
    ...
    lteSimpleHelper->m_ueMac->SetTxOpportunityMode(NrTestMac::RANDOM_MODE);

    // Start/Stop pseudo-application at eNB RRC
    Simulator::Schedule(Seconds(0.100), &NrTestRrc::Start, lteSimpleHelper->m_enbRrc);
    Simulator::Schedule(Seconds(10.100), &NrTestRrc::Stop, lteSimpleHelper->m_enbRrc);

    // Start/Stop pseudo-application at UE RRC
    Simulator::Schedule(Seconds(20.100), &NrTestRrc::Start, lteSimpleHelper->m_ueRrc);
    Simulator::Schedule(Seconds(30.100), &NrTestRrc::Stop, lteSimpleHelper->m_ueRrc);
