...
NrX2HandoverTestCase::DoRun()
{
    ...
    enbDevices = m_nrHelper->InstallGnbDevice(enbNodes, bandwidthAndBWPPair.second);
    ...
    m_nrHelper->UpdateDeviceConfigs(enbDevices);
    ...
    {
        m_nrHelper->AttachToEnb(ueDevices.Get(i), enbDevices.Get(0));
    }
         ...
         ++hoEventIt)
    {
        ...
        Simulator::Schedule(hoEventIt->startTime + MilliSeconds(40),
                            &NrX2HandoverTestCase::TeleportUeNearTargetEnb,
                            this,
                            ueNodes.Get(hoEventIt->ueDeviceIndex),
...
NrX2HandoverTestCase::CheckConnected(Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
    ...
    std::vector<uint16_t> enbCellId = enbNrDevice->GetCellIds();
    bool enbCellIdFound =
        std::find(enbCellId.begin(), enbCellId.end(), ueCellId) != enbCellId.end();
    NS_TEST_ASSERT_MSG_EQ(enbCellIdFound, true, "gNB does not contain UE cellId");
...
void
NrX2HandoverTestCase::TeleportUeNearTargetEnb(Ptr<Node> ueNode, Ptr<Node> enbNode)
{
 ...
 */
static NrX2HandoverTestSuite g_lteX2HandoverTestSuiteInstance;
