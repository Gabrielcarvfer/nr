...
EpcS1uDlTestCase::DoRun()
{
    ...
    for (auto enbit = m_enbDlTestData.begin(); enbit < m_enbDlTestData.end(); ++enbit)
    {
        ...
        // Note that the NrEpcEnbApplication won't care of the actual NetDevice type
        ...
        NS_ASSERT_MSG(enbApp, "cannot retrieve NrEpcEnbApplication");
        ...
        for (uint32_t u = 0; u < ues.GetN(); ++u)
        {
            ...
            epcHelper->AddUe(ueNrDevice, imsi);
            epcHelper->ActivateEpsBearer(ueNrDevice,
                                         imsi,
                                         NrEpcTft::Default(),
                                         NrEpsBearer(NrEpsBearer::NGBR_VIDEO_TCP_DEFAULT));
            Simulator::Schedule(MilliSeconds(10),
                                &NrEpcEnbS1SapProvider::InitialUeMessage,
