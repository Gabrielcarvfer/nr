...
void
NrIpv6RoutingTestCase::EnbToPgw(Ptr<Packet> p)
{
    Ipv6Header ipv6Header;
    p->PeekHeader(ipv6Header);
    if (ipv6Header.GetNextHeader() == UdpL4Protocol::PROT_NUMBER)
...
NrIpv6RoutingTestCase::Checker()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(m_pgwUidRxFrmEnb.size(), 0, "Route is not Redundant in Nr IPv6 test");
...
NrIpv6RoutingTestCase::DoRun()
{
    ...
    NetDeviceContainer enbNrDevs = nrHelper->InstallGnbDevice(enbNodes, bandwidthAndBWPPair.second);
    NetDeviceContainer ueNrDevs = nrHelper->InstallUeDevice(ueNodes, bandwidthAndBWPPair.second);
    nrHelper->UpdateDeviceConfigs(enbNrDevs);
    nrHelper->UpdateDeviceConfigs(ueNrDevs);

    // Assign IP address to UEs, and install applications
    m_ueIpIface = epcHelper->AssignUeIpv6Address(NetDeviceContainer(ueNrDevs));
    ...
    nrHelper->AttachToEnb(ueNrDevs.Get(0), enbNrDevs.Get(0));
    nrHelper->AttachToEnb(ueNrDevs.Get(1), enbNrDevs.Get(0));
    nrHelper->AttachToEnb(ueNrDevs.Get(2), enbNrDevs.Get(1));
    ...
    appPgw->TraceConnectWithoutContext("RxFromS1u",
                                       MakeCallback(&NrIpv6RoutingTestCase::EnbToPgw, this));
 ...
 */
static NrIpv6RoutingTestSuite g_lteipv6testsuite;
