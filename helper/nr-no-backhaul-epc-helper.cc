                              ...
                              std::vector<uint16_t> cellIds)
{
    ...
    enbNrSocketConnectAddress.SetSingleDevice(nrGnbNetDevice->GetIfIndex());
    enbNrSocketConnectAddress.SetProtocol(Ipv4L3Protocol::PROT_NUMBER);
    retval = enbNrSocket->Connect(enbNrSocketConnectAddress);
    NS_ASSERT(retval == 0);

    // create LTE socket for the ENB
    Ptr<Socket> enbNrSocket6 =
        Socket::CreateSocket(enb, TypeId::LookupByName("ns3::PacketSocketFactory"));
    PacketSocketAddress enbNrSocketBindAddress6;
    enbNrSocketBindAddress6.SetSingleDevice(nrGnbNetDevice->GetIfIndex());
    enbNrSocketBindAddress6.SetProtocol(Ipv6L3Protocol::PROT_NUMBER);
    retval = enbNrSocket6->Bind(enbNrSocketBindAddress6);
    NS_ASSERT(retval == 0);
    PacketSocketAddress enbNrSocketConnectAddress6;
    enbNrSocketConnectAddress6.SetPhysicalAddress(Mac48Address::GetBroadcast());
    enbNrSocketConnectAddress6.SetSingleDevice(nrGnbNetDevice->GetIfIndex());
    enbNrSocketConnectAddress6.SetProtocol(Ipv6L3Protocol::PROT_NUMBER);
    retval = enbNrSocket6->Connect(enbNrSocketConnectAddress6);
    NS_ASSERT(retval == 0);

    NS_LOG_INFO("Create NrEpcEnbApplication for cell ID " << cellIds.at(0));
    Ptr<NrEpcEnbApplication> enbApp =
        CreateObject<NrEpcEnbApplication>(enbNrSocket, enbNrSocket6, cellIds.at(0));
    enb->AddApplication(enbApp);
    NS_ASSERT(enb->GetNApplications() == 1);
    NS_ASSERT_MSG(enb->GetApplication(0)->GetObject<NrEpcEnbApplication>(),
                  "cannot retrieve NrEpcEnbApplication");
...
void
NrNoBackhaulEpcHelper::AddX2Interface(Ptr<Node> enb1, Ptr<Node> enb2)
{
    ...
    Ptr<NetDevice> enb1NrDev = enb1->GetDevice(0);
    Ptr<NetDevice> enb2NrDev = enb2->GetDevice(0);

    DoAddX2Interface(enb1X2, enb1NrDev, enb1X2Address, enb2X2, enb2NrDev, enb2X2Address);
}

void
NrNoBackhaulEpcHelper::DoAddX2Interface(const Ptr<NrEpcX2>& enb1X2,
                                        const Ptr<NetDevice>& enb1NrDev,
                                        const Ipv4Address& enb1X2Address,
                                        const Ptr<NrEpcX2>& enb2X2,
                                        const Ptr<NetDevice>& enb2NrDev,
                                        const Ipv4Address& enb2X2Address) const
{
    NS_LOG_FUNCTION(this);

    Ptr<NrGnbNetDevice> enb1NrDevice = enb1NrDev->GetObject<NrGnbNetDevice>();
    Ptr<NrGnbNetDevice> enb2NrDevice = enb2NrDev->GetObject<NrGnbNetDevice>();

    NS_ABORT_MSG_IF(!enb1NrDevice, "Unable to find NrGnbNetDevice for the first eNB");
    NS_ABORT_MSG_IF(!enb2NrDevice, "Unable to find NrGnbNetDevice for the second eNB");

    std::vector<uint16_t> enb1CellIds = enb1NrDevice->GetCellIds();
    std::vector<uint16_t> enb2CellIds = enb2NrDevice->GetCellIds();
    ...
    enb1NrDevice->GetRrc()->AddX2Neighbour(enb2CellId);
    enb2NrDevice->GetRrc()->AddX2Neighbour(enb1CellId);
}
...
void
NrNoBackhaulEpcHelper::AddS1Interface(Ptr<Node> enb,
                                      ...
                                      std::vector<uint16_t> cellIds)
{
    ...
    Ptr<NrEpcEnbApplication> enbApp = enb->GetApplication(0)->GetObject<NrEpcEnbApplication>();
    NS_ASSERT_MSG(enbApp, "NrEpcEnbApplication not available");
