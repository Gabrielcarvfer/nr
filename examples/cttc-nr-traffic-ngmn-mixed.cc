...
main(int argc, char* argv[])
{
    ...
    auto remoteHostContainer = NodeContainer(remoteHostAndIpv4address.first);
    ...
    {
        serverApps.Add(dlPacketSinkLowLat.Install(remoteHostAndIpv4address.first));
    }
    ...
    if (trafficTypeConf == NGMN_FTP || (trafficTypeConf == NGMN_MIXED && !ngmnFtpIds.empty()))
    {
        ...
        if (direction == "DL")
        {
            ...
            for (uint32_t i = 0; i < ueSector1IpIface.GetN(); i++)
            {
                ...
                clientApps.Add(ftpHelper.Install(remoteHostAndIpv4address.first));
                // Seed the ARP cache by pinging early in the simulation
                // This is a workaround until a static ARP capability is provided
                PingHelper ping(ipAddress);
                pingApps.Add(ping.Install(remoteHostAndIpv4address.first));
            }
    ...
    if (trafficTypeConf == NGMN_VIDEO || (trafficTypeConf == NGMN_MIXED && !ngmnVideoIds.empty()))
    {
        ...
        if (direction == "DL")
        {
            ...
            for (uint32_t i = 0; i < ueSector1IpIface.GetN(); i++)
            {
                ...
                clientApps.Add(trafficGeneratorHelper.Install(remoteHostAndIpv4address.first));
                // Seed the ARP cache by pinging early in the simulation
                // This is a workaround until a static ARP capability is provided
                PingHelper ping(ipAddress);
                pingApps.Add(ping.Install(remoteHostAndIpv4address.first));
    ...
    if (trafficTypeConf == NGMN_HTTP || (trafficTypeConf == NGMN_MIXED && !ngmnHttpIds.empty()))
    {
        ...
        ThreeGppHttpClientHelper clientHelper(remoteHostAndIpv4address.second);
        ...
        ThreeGppHttpServerHelper serverHelper(remoteHostAndIpv4address.second);
        // Install HTTP server on a remote host node
        ApplicationContainer serverApps = serverHelper.Install(remoteHostAndIpv4address.first);
    ...
    if (trafficTypeConf == UDP_CBR)
    {
        for (uint32_t i = 0; i < ueSector1Container.GetN(); ++i)
        {
            ...
            {
                dlClientLowLat.SetAttribute("RemoteAddress",
